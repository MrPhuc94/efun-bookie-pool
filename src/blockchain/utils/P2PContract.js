...
const p2pABI =
  network === "TESTNET"
    ? require("./contracts/p2p.abi.json")
    : require("./contracts/p2p.abi.json");
const p2pContractAddress =
  process.env.NODE_ENV === "development"
    ? process.env.P2P_CONTRACT
    : process.env.P2P_CONTRACT_MAINNET;
...
export const getMatchInfo = async (matchId) => {
  ...
  const p2pContract = await new web3.eth.Contract(p2pABI, p2pContractAddress);
  const txData = await p2pContract.methods.getMatchInfo(matchId).call();
  const tx = {
...
export const createPrediction = async (
  ...
  if (_token === process.env.BNB_TOKEN) {
    param.value = new BigNumber(_hardCap).multipliedBy(10 ** 18).toFixed();
  }
  // console.log(_token, "token");
  const txData = await p2pContract.methods
    .createPrediction(
      matchId,
      _token,
      new BigNumber(_hardCap).multipliedBy(10 ** 18).toFixed(),
      new BigNumber(minBet).multipliedBy(10 ** 18).toFixed(),
      handicap,
      chosenTeam
    )
    .send(param)
...
export const predict = async (_predictionId, value, token, from) => {
  const web3 = await initWeb3();
  const p2pContract = await new web3.eth.Contract(p2pABI, p2pContractAddress);
  let receipt2 = "";
  ...
  if (token === "BNB") {
    param.value = new BigNumber(value).multipliedBy(10 ** 18).toFixed();
  }
  const txData = await p2pContract.methods
    .predict(
      _predictionId,
      new BigNumber(value).multipliedBy(10 ** 18).toFixed()
    )
...
export const getPredictions = async (idx) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  const data = await p2pContract.methods.predictions(idx).call();
  return data;
};
export const dealerWithdraw = async (_predictionId, from) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  let receipt2 = "";
  //console.log(_predictionId, from);
  const txData = await p2pContract.methods
    .dealerWithdraw(_predictionId)
    .send({ from: from })
    .on("error", (error) => {
      receipt2 = error;
      //console.log(error);
...
export const cancelPrediction = async (_predictionId, from) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  let receipt2 = "";
  //console.log(_predictionId, from);
  ...
  const txData = await p2pContract.methods
    .cancelPrediction(_predictionId)
    .send({ from: from })
    .on("error", (error) => {
      receipt2 = error;
      // console.log("error", error);
...
export const predictionStats = async (_predictionId, from) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  //console.log(_predictionId, from);
  // const accounts = await web3.eth.getAccounts()
  const txData = await p2pContract.methods
    .predictionStats(from, _predictionId)
    .call();
  // const nonce = await web3.eth.getTransactionCount(from, 'pending')
  const tx = {
    from,
...
export const getBetInfo = async (matchId, account) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  const txData = await p2pContract.methods.getBetInfo(matchId, account).call();
...
export const calculateReward = async (
  ...
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  // console.log(score, handicap, amount, option, account, "score,112");
  const txData = await p2pContract.methods
    .calculateReward(
      score,
      handicap,
...
export const claimReward = async (_predictionId, from) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  let receipt2 = "";
  // const accounts = await web3.eth.getAccounts()
  const txData = await p2pContract.methods
    .claimReward(_predictionId)
    .send({ from: from })
    .on("error", (error) => {
      receipt2 = error;
      // console.log(error);
...
export const updateResult = async (matchId, result, account) => {
  const web3 = await initWeb3();
  const p2pContract = new web3.eth.Contract(p2pABI, p2pContractAddress);
  // console.log(result, "result");
  // const accounts = await web3.eth.getAccounts()
  const txData = await p2pContract.methods.updateResult(matchId, result);
  // const nonce = await web3.eth.getTransactionCount(account, 'pending')
  const tx = {
    from: account,
    to: p2pContractAddress,
