...
const groupAbi = process.env.NODE_ENV === 'development' ? require('./contracts/group-test.abi.json') : require('./contracts/group.abi.json')
const createAbi = process.env.NODE_ENV === 'development' ? require('./contracts/create.abi.json') : require('./contracts/create.abi.json')
...
const groupContract = process.env.NODE_ENV === 'development' ? process.env.SPONSOR_PREDICT_DEV : process.env.SPONSOR_PREDICT_DEV
const createContract = process.env.NODE_ENV === 'development' ? process.env.SPONSOR_MATCH_DEV : process.env.SPONSOR_MATCH_DEV
...
export const getMatchInfo2 = async (matchId) => {
  const web3 = await initWeb3()
  const tokenContract = await new web3.eth.Contract(
    createAbi,
    groupContract
  )
  const txData = await tokenContract.methods.info(
    matchId
  ).call()
  const tx = {
    to: groupContract,
...
export const getBetInfo = async (
  ...
  const txData = await tokenContract.methods.getPredictInfo(
    matchId,
    account,
    token
  ).call()
...
export const getMatchInfo = async (matchId, token) => {
  const web3 = await initWeb3()
  const tokenContract = await new web3.eth.Contract(
    groupAbi,
    groupContract
  )
  const tokenContract2 = await new web3.eth.Contract(
    createAbi,
    createContract
  )
  const txData = await tokenContract.methods.getMatchInfo(
    matchId,
    token
  ).call()
  const txData2 = await tokenContract2.methods.info(
    matchId
  ).call()
  const dataCombine = {
...
export const createMatches = async (
  ...
  const txData = await tokenContract.methods.createMatches(
    x,
    _startTimes,
    _endTimes,
    _sToken
...
export const createMatchs = async (
  matchIds,
  minBet,
  startTime,
  endTime,
  ...
  const accounts = await web3.eth.getAccounts()
  const txData = await tokenContract.methods.createMatchs(
    matchIds,
    minBet,
    startTime,
    endTime,
...
export const predict = async (
  ...
  const txData = await tokenContract.methods.predict(
    matchId,
    betContent,
    token,
    new BigNumber(
...
export const cancelMatchs = async (
  matchId,
  from
) => {
  const web3 = await initWeb3()
  ...
  // const accounts = await web3.eth.getAccounts()
  const txData = await tokenContract.methods.cancelMatchs(
    matchId
  ).call()
  const nonce = await web3.eth.getTransactionCount(from, 'pending')
  const tx = {
...
export const calculateReward = async (
  ...
  const txData = await tokenContract.methods.calculateReward(
    matchId,
    account,
    token
  ).call()
...
export const claimReward = async (
  ...
  const txData = await tokenContract.methods.claimReward(
    matchId,
    _token
  )
    // .call()
...
export const updateResult = async (
  ...
  // const accounts = await web3.eth.getAccounts()
  const txData = await tokenContract.methods.updateResult(
    matchId,
    result
  )
  // const nonce = await web3.eth.getTransactionCount(account, 'pending')
 ...
 * @returns
 */
export const checkApproveTx = async (from, tokenSymbol, spender = groupContract) => {
  const web3 = await initWeb3()
 ...
 * @returns
 */
export const createApproveTx = async (from, tokenSymbol, amount = MAX_INT, spender = groupContract) => {
  const web3 = await initWeb3()
  const tokenContract = new web3.eth.Contract(
    erc20Abi,
